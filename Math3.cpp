//g++ Math3.cpp glad.c stb_image.cpp -Iincludes -o app.exe -lopengl32 -lgdi32
#include <glad/glad.h>
#include <glm/glm.hpp>
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Math test", NULL, NULL);
    if (window == NULL)
    {
    ...
    const std::string fragmentShader = "/Shaders/Image.frag";
...
double * mouse = (double *) malloc(sizeof(double)*4);
    ...
    glBindTexture(GL_TEXTURE_2D, luaTexture);
       ...
       // Imageprogram.setSampler("iChannel2",2);
        ...
        sprintf(title_string, "Math test - FPS = %.i ", (int)fps);
